...
Official mariadb plugin for dokku. Currently defaults to installing [mariadb 10.11.2](https://hub.docker.com/_/mariadb/).
...
```
DATABASE_URL=mysql://lollipop:SOME_PASSWORD@dokku-mariadb-lollipop:3306/lollipop
```
...
```
mysql2://lollipop:SOME_PASSWORD@dokku-mariadb-lollipop:3306/lollipop
```

If you specify `MARIADB_DATABASE_SCHEME` to equal `http`, we`ll also automatically adjust `DATABASE_URL` to match the http interface:

```
http://lollipop:SOME_PASSWORD@dokku-mariadb-lollipop:${PLUGIN_DATASTORE_PORTS[1]}
```
