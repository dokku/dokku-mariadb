...
@test "($PLUGIN_COMMAND_PREFIX:info) success" {
  ...
  assert_contains "${lines[*]}" "DSN: mysql://mariadb:$password@dokku-mariadb-l:3306/l"
}
...
@test "($PLUGIN_COMMAND_PREFIX:info) replaces underscores by dash in hostname" {
  ...
  assert_contains "${lines[*]}" "DSN: mysql://mariadb:$password@dokku-mariadb-test-with-underscores:3306/test_with_underscores"
